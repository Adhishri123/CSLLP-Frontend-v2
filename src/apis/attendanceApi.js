...
export const importDailyAttendance = async () => {
  const response = await axiosInstance.post(`${API_URL}/import/daily`);
  return response.data;
};

// ✅ Get today's attendance (GET from your own DB after import)
export const getDailyAttendance = async (date) => {
  const response = await axiosInstance.get(`${API_URL}/daily/${date}`);
  return response.data;
};

// ✅ Import attendance by date
export const importAttendanceByDate = async (date) => {
  const response = await axiosInstance.post(`${API_URL}/import/by-date?date=${date}`);
  return response.data;
};
...
export const getMonthlyReportJson = async (monthYear, employeeIds) => {
  const response = await axiosInstance.post(`${API_URL}/monthly-report/json`, {
    monthYear,
    employeeIds,
  });
  return response.data;
...
export const getPresentAbsentSummary = async (date) => {
  const url = date
    ? `${API_URL}/present-absent-summary?date=${date}` 
    : `${API_URL}/present-absent-summary`; 
...
export const registerEmployee = async (employeeData) => {
  const response = await axiosInstance.post(`${EMPLOYEE_API_URL}/register`, employeeData);
  return response.data;
};

// Update employee details
export const updateEmployee = async (id, employeeData) => {
  const response = await axiosInstance.put(`${EMPLOYEE_API_URL}/${id}`, employeeData);
  return response.data;
};

// Get employee details by ID
export const getEmployeeById = async (id) => {
  const response = await axiosInstance.get(`${EMPLOYEE_API_URL}/${id}`);
  return response.data; 
};

// Get all employees
export const getAllEmployees = async () => {
  const response = await axiosInstance.get(`${EMPLOYEE_API_URL}/all`);
  return response.data;
};